...
def start():
    ...
    scheduler.add_job(tasks.update_items,args=['job'],trigger="interval",minutes=5,id="FetchJobTaskid",replace_existing=True,misfire_grace_time=None,next_run_time=now)# add task to backgroud scheduler and run at intervals
    scheduler.start()