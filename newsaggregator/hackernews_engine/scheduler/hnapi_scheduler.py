...
def start():
    ...
    now = datetime.now()
    scheduler.add_job(tasks.update_items,args=['top'],trigger="interval",minutes=10,id="FetchTopTaskid",replace_existing=True,misfire_grace_time=None,next_run_time=now)# add task to backgroud scheduler and run at intervals
